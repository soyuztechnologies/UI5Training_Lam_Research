...
function jQMagic(){
    $(".box-content").css("background", "pink").css("color", "black");
}
function hide(){
    $(".box").fadeOut(5000, function(){
        alert("effect is complete");
    });    
}
function attach(){
    $("#attack").click(this.attack);
}
function attack(){
    $(".box").toggle();
}
    ...
    $.ajax("https://api.covidtracking.com/v2/us/daily.json",{
        success: function(data){
            ...
            for (let i = 0; i < aData.length; i++) {
                ...
                var sDate = element.date;
                var sConfirmed = element.cases.total.value;
                var sVal = sDate + " - " + sConfirmed;
                addElement(sVal);
            }
        }
    })
